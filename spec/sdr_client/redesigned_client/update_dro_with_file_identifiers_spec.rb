...
RSpec.describe SdrClient::RedesignedClient::UpdateDroWithFileIdentifiers do
  ...
  context 'when file not included in upload responses' do
    let(:upload_responses) do
      [
        SdrClient::RedesignedClient::DirectUploadResponse.new(filename: '00002.html', signed_id: 'abc123')
      ]
    end
  ...
  context 'when file included in upload responses' do
    let(:upload_responses) do
      [
        SdrClient::RedesignedClient::DirectUploadResponse.new(filename: '00001.html', signed_id: 'abc123')
      ]
    end
